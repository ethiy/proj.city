...
namespace urban
{
    ...
    //         {
    //             norm = std::sqrt(angle_axis.second * angle_axis.second);
    //             u = angle_axis.second / norm;
    //             return Affine_transformation(
    //                     std::cos(angle_axis.first) + (1 - std::cos(angle_axis.first)) * std::pow(u.x(), 2.), 
    //                     (1 - std::cos(angle_axis.first)) * u.x() * u.y() - std::sin(angle_axis.first) * u.z(), 
    //                     (1 - std::cos(angle_axis.first)) * u.x() * u.z() + std::sin(angle_axis.first) * u.y(), 
    //                     (1 - std::cos(angle_axis.first)) * u.x() * u.y() + std::sin(angle_axis.first) * u.z(), 
    //                     std::cos(angle_axis.first) + (1 - std::cos(angle_axis.first)) * std::pow(u.y(), 2.), 
    //                     (1 - std::cos(angle_axis.first)) * u.y() * u.z())- std::sin(angle_axis.first) * u.x(), 
    //                     (1 - std::cos(angle_axis.first)) * u.x() * u.z() - std::sin(angle_axis.first) * u.y(), 
    //                     (1 - std::cos(angle_axis.first)) * u.y() * u.z() + std::sin(angle_axis.first) * u.x(), 
    //                     std::cos(angle_axis.first) + (1 - std::cos(angle_axis.first)) * std::pow(u.z(), 2.)
    //                 );
    //         }
    //     );
    ...
    void extrem_points(std::vector<Point_2> & points)
    {
        ...
        std::for_each(
            ...
            [&](Point_2 C)
            {
                AB = CGAL::squared_distance(A, B);
                AC = CGAL::squared_distance(A, C);
                BC = CGAL::squared_distance(B, C);
                if(AC/AB > 1 || BC/AB > 1)
                {
                    if(AB/AC > 1 || BC/AC > 1)
                        A = C;
