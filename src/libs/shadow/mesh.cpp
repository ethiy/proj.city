...
namespace urban
{
    namespace shadow
    {
        ...
        Mesh::Mesh(Lib3dsMesh* lib3ds_mesh):name(lib3ds_mesh->name)
        {
            ...
            std::for_each(
                ...
                [&it, this](Lib3dsFace _face)
                {
                    ...
                    if(determinant(v1, v2, n) > std::numeric_limits<double>::epsilon())
        ...
        {
            bool equal(false);
            if(lhs.points_size() == rhs.points_size() && lhs.faces_size() == rhs.faces_size())
            {
                std::vector<bool> points_equalities(lhs.points_size());
                std::vector<bool> faces_equalities(lhs.faces_size());
                std::transform(
                    lhs.points_cbegin(),
                    lhs.points_cend(),
                    rhs.points_cbegin(),
