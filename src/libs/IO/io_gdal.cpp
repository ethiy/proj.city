...
namespace urban
{
    namespace io
    {
        ...
        const  std::list<std::string> tested_raster_formats{{"BMP", "GTiff", "JP2OpenJPEG", "JP2KAK"}};
        ...
        projection::BrickPrint FileHandler<GDALDriver>::read_vectorial(void)
        {
        ...
        std::vector<double> FileHandler<GDALDriver>::read_raster(void)
        {
        ...
        void FileHandler<GDALDriver>::write(const std::vector<uint16_t> & raster_image, const std::pair<size_t, size_t> image_sizes)
        {
            ...
            if (modes["write"])
            {
                if(raster)
                {
                    ...
                    uint16_t gdal_image[raster_image.size()];
                    std::copy(std::begin(raster_image), std::end(raster_image), gdal_image);
                    unique_band->RasterIO(GF_Write, 0, 0, image_sizes.second, image_sizes.first, gdal_image, image_sizes.second, image_sizes.first, GDT_UInt16,0, 0);
